...
.NOTES
    Updated: 2022-09-25     Added support for SOX for adding an endfile. //EZS
...
$ReqIDDiscord = $Destination_dir + "\templatefiles\intreqdiscord.mp3"
$OutReqID = $Destination_dir + "\templatefiles\outreq.wav"
$Slots = 2
$Password = "DEADBEEFD3ADB33F1GGGAACC45353689090"
...
try {
    ...
    $Source = $NextSong.Requests[0].Source
    if ($Source -eq $null) { $Source = "Internet" }
    elseif ($Source -eq "Discord") { $ReqID = $ReqIDDiscord }
...
try {
    ...
    #$cmdline =  "$ffmpeg_exe -i `"" + $ReqID + "`" -i `"" + $sourcepath + "`" -i `"" + $OutReqID + "`" -metadata title=`""+ $washedtitle + "`" -metadata artist=`"" + "Listener-request: " + $washedartist + "`" -filter_complex '[0:0][1:0][2:0]concat=n=3:v=0:a=1[out]' -map '[out]' `"" + ($Destination_dir+"\"+$ReqFileFull) + "`" "
    ...
    $LENGTH = $CueOut - 10 # Fade point (CUE OUT)
    ...
    $cmdline = "$ffmpeg_exe -i `"" + $ReqID + "`" -i `"" + $out + "`" -i `"" + $OutReqID + "`" -metadata title=`"" + $washedtitle + "`" -metadata artist=`"" + "Listener-request: " + $washedartist + "`" -filter_complex '[0:0][1:0][2:0]concat=n=3:v=0:a=1[out]' -map '[out]' `"" + ($Destination_dir + "\" + $ReqFileFull) + "`" "
    #Write-Verbose "`t CMD: $cmdline"
...
try {
    ...
    if ($CallResult.subcode -eq "TRUE") { logwrite ("The tune has been set as played in the API."); } else { logwrite ("[ERROR] Failed to advance the tune. Given EC: " + $_ + "") }
